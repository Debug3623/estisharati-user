...
import estisharati.bussiness.eshtisharati_consultants.Utils.PinOnKeyListener
import estisharati.bussiness.eshtisharati_consultants.Utils.PinTextWatcher
...
class ForgotPassword : BaseCompatActivity() {
    ...
    fun clickEvents() {
        ...
        btn_proceed.setOnClickListener {
            if (codeValidation()) {
                val code =
                    "${code_1.toText()}${code_2.toText()}${code_3.toText()}${code_4.toText()}${code_5.toText()}${code_6.toText()}"
                if (vrCode == null) {
        ...
        save_passwrod.setOnClickListener {
            if (changePasswordValidation()) {
                val code =
                    "${code_1.toText()}${code_2.toText()}${code_3.toText()}${code_4.toText()}${code_5.toText()}${code_6.toText()}"
                VerifyResetCodeApiCall(
                    codePicker.selectedCountryCodeWithPlus + "" + phone.toText(),
                    code,
                    new_password.toText()
                )
            }
    ...
    fun ResendCountdown() {
        ...
        resendTimer = object : CountDownTimer(120000, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                retry_on.text =
                    getString(R.string.retry_on) + " " + helperMethods.MillisUntilToTime(
                        millisUntilFinished
                    )
            }
    ...
    fun changePasswordValidation(): Boolean {
        ...
        if (!helperMethods.isValidPassword(new_password.toText())) {
            helperMethods.AlertPopup(
                ...
                getString(R.string.password_at_least_8_characters_including_a_lower_case_letteran_uppercase_lettera_number_and_one_special_character)
            )
            return false
        }
        ...
        if (new_password.toText() != confirm_password.toText()) {
...
//                                helperMethods.setStatusBarColor(this@ForgotPassword, R.color.orange)
    ...
    private fun EditText.toText(): String = text.toString().trim()
