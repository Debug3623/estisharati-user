...
class ActivitySplashScreen : BaseCompatActivity() {
    ...
    private fun checkUpdateAvailable() {
        appUpdateManager = AppUpdateManagerFactory.create(this@ActivitySplashScreen)
        appUpdateManager!!.appUpdateInfo.addOnSuccessListener(OnSuccessListener { appUpdateInfo: AppUpdateInfo ->
            if (appUpdateInfo.updateAvailability() == UpdateAvailability.UPDATE_AVAILABLE && appUpdateInfo.isUpdateTypeAllowed(AppUpdateType.IMMEDIATE)) {
                try {
                    appUpdateManager!!.startUpdateFlowForResult(appUpdateInfo, AppUpdateType.IMMEDIATE, this@ActivitySplashScreen, UPDATE_REQUEST)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == UPDATE_REQUEST) {
            if (resultCode == RESULT_OK) {
                Toast.makeText(this@ActivitySplashScreen, getString(R.string.app_update_start), Toast.LENGTH_LONG).show()
                Log.d("inAppUpdate", getString(R.string.app_update_start))
            } else if (resultCode == RESULT_CANCELED) {
                Toast.makeText(this@ActivitySplashScreen, getString(R.string.app_update_canceled), Toast.LENGTH_LONG).show()
                Log.d("inAppUpdate", getString(R.string.app_update_canceled))
                finish()
            } else if (resultCode == ActivityResult.RESULT_IN_APP_UPDATE_FAILED) {
                Toast.makeText(this@ActivitySplashScreen, getString(R.string.app_update_failed), Toast.LENGTH_LONG).show()
