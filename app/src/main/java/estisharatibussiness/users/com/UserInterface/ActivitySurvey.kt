...
import estisharatibussiness.users.com.DataClassHelper.SubmitSurvey.Question
import estisharatibussiness.users.com.DataClassHelper.SubmitSurvey.SubmitSurvey
import estisharatibussiness.users.com.DataClassHelper.SurveysQuestions.SurveysQuestionsResponse
...
class ActivitySurvey : AppCompatActivity() {
    ...
    fun initViews() {
        helperMethods = HelperMethods(this@ActivitySurvey)
        retrofitInterface = RetrofitApiClient(GlobalData.BaseUrl).getRetrofit().create(RetrofitInterface::class.java)
        sharedPreferencesHelper = SharedPreferencesHelper(this@ActivitySurvey)
    ...
    fun clickEvents() {
        ...
        goToHome.setOnClickListener {
            val intent = Intent(this@ActivitySurvey, UserDrawerActivity::class.java)
    ...
    fun checkPreviousNext() {
        if (currentPosition == 0) {
            previousQuestion.setColorFilter(ContextCompat.getColor(this@ActivitySurvey, R.color.dark_gray))
        } else {
            previousQuestion.clearColorFilter()
        }
        if (currentPosition == surveysQuestionsResponse.data.questions.lastIndex) {
            nextQuestion.setColorFilter(ContextCompat.getColor(this@ActivitySurvey, R.color.dark_gray))
        } else {
    ...
    fun setSurveyDetailsApi() {
        ...
        Glide.with(this@ActivitySurvey).load(surveysQuestionsResponse.data.image_path).apply(helperMethods.requestOption).into(surveyImage)
    }

    fun createRadioButton() {
        currentPosition
        questionTxt.text = surveysQuestionsResponse.data.questions.get(currentPosition).question
        Glide.with(this@ActivitySurvey).load(surveysQuestionsResponse.data.questions.get(currentPosition).image_path).apply(helperMethods.requestOption).into(questionImage)
        questionRadioGroup.removeAllViews()
        for (option in surveysQuestionsResponse.data.questions.get(currentPosition).options) {
            val rdbtn = RadioButton(this@ActivitySurvey)
            ...
            rdbtn.setTextColor(ContextCompat.getColor(this@ActivitySurvey, R.color.black))
            rdbtn.textSize = 16f
            ...
            if (option.image_path.equals("")) {
                ...
                Glide.with(this@ActivitySurvey).load(option.image_path).apply(helperMethods.requestOption).into(object : CustomTarget<Drawable>(200, 150) {
