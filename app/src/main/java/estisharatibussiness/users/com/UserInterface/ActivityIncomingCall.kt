...
import estisharatibussiness.users.com.DataClassHelper.Chat.DataCallsFireStore
...
class ActivityIncomingCall : BaseCompatActivity() {
    ...
    fun initViews() {
        helperMethods = HelperMethods(this@ActivityIncomingCall)
        preferencesHelper = SharedPreferencesHelper(this@ActivityIncomingCall)
    ...
    fun setDetails() {
                ...
                when (dataCallsFireStore.call_status) {
                    "accept" -> {
                        ...
                        if (dataCallsFireStore.call_type.equals("voice_call")) {
                            val intent = Intent(this@ActivityIncomingCall, VoiceCallActivity::class.java)
                            intent.putExtra("audio_balance", audio_balance)
                            startActivity(intent)
                            finish()
                        } else if (dataCallsFireStore.call_type.equals("video_call")) {
                            val intent = Intent(this@ActivityIncomingCall, VideoCallActivity::class.java)
        ...
        Glide.with(this@ActivityIncomingCall).load(dataOtherUserFireStore.image).apply(helperMethods.profileRequestOption).into(caller_profile)
    ...
    fun clickEvents() {
        call_accept.setOnClickListener {
            if (ContextCompat.checkSelfPermission(this@ActivityIncomingCall, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED && ContextCompat.checkSelfPermission(this@ActivityIncomingCall, Manifest.permission.CAMERA) == PackageManager.PERMISSION_GRANTED) {
                try {
                    if (player!!.isPlaying) {
                        player?.stop()
                        player?.release()
                ...
                helperMethods.selfPermission(this@ActivityIncomingCall)
            }
        }
    ...
    override fun onRequestPermissionsResult(requestCode: Int, permissions: Array<out String>, grantResults: IntArray) {
        ...
        if (ContextCompat.checkSelfPermission(this@ActivityIncomingCall, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_DENIED || ContextCompat.checkSelfPermission(this@ActivityIncomingCall, Manifest.permission.CAMERA) == PackageManager.PERMISSION_DENIED) {
