package digital.upbeat.estisharati_user.MessageSwipe
...
import digital.upbeat.estisharati_user.MessageSwipe.AndroidUtils
...
class MessageSwipeController(private val context: Context, private val swipeControllerActions: SwipeControllerActions) : ItemTouchHelper.Callback() {
    ...
    private fun drawReplyButton(canvas: Canvas) {
        ...
        shareRound.setBounds((x - convertTodp(18) * scale).toInt(), (y - convertTodp(18) * scale).toInt(), (x + convertTodp(18) * scale).toInt(), (y + convertTodp(18) * scale).toInt())
        shareRound.draw(canvas)
        imageDrawable.setBounds((x - convertTodp(12) * scale).toInt(), (y - convertTodp(11) * scale).toInt(), (x + convertTodp(12) * scale).toInt(), (y + convertTodp(10) * scale).toInt())
